...
For passive concentrated liquidity pools, this fee is variable and ranges depending on the instantiate parameters for that particular pool (ex: 0.26%-0.45%, or 0.03%-0.45% for some liquid staking pairs, etc.), divided as follows:

- 50% of the total fee goes to the LPs for the relevant pool (75% on Osmosis outpost) — this remains denominated in the native pool tokens and acts as a payment by increasing the size of the pools (thus increasing the number of tokens that LPs can redeem when they remove liquidity); and
- 50% of the total fee goes to the Astral Assembly (25% on Osmosis outpost) — this is used to purchase ASTRO from ASTRO liquidity pools. The purchased ASTRO goes into the xASTRO staking pool per a ratio set by the Astral Assembly.
