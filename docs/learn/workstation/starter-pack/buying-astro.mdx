...
There are two primary ways to obtain ASTRO:

1. Centralized approach: Use a CEX to buy the native token on any chain where Astroport has an active outpost: Neutron (NTRN), Terra (LUNA), Sei (SEI) or Injective (INJ). Then, withdraw it to a wallet on your target chain and swap that token for $ASTRO at app.astroport.fi.
2. Decentralized approach: Bridge tokens to a chain where Astroport has an active outpost with Squid Router, IBC.fun or TFM.com. Then, swap those bridged tokens for $ASTRO at app.astroport.fi.
...
- **CCTP:** Purchase $USDC and withdraw to an EVM chain or Noble directly (availiable on Coinbase). Bridge your USDC to 
