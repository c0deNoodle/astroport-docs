...
The Emissions Controller Outpost exists on each chain that Astroport is deployed and functions as a lightweight "survey satellite" for the main [Emissions Controller (Hub)](Emissions%20Controller%20(Hub).mdx). From the vxASTRO voter's perspective, this contract has the same features as the main Emissions Controller. However, the Outpost is more limited and cannot perform fine-grained sanity checks for voted LP tokens.
